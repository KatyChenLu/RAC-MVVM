...
typedef enum  {
    BBSnapshotMeetsEdgeTop,
    BBSnapshotMeetsEdgeBottom,
}BBSnapEdge;
@interface BBDragTable()
/**对被选中的cell的截图*/
...
/**自动滚动的方向*/
@property (nonatomic, assign) BBSnapEdge autoScrollDirection;

@end

@implementation BBDragTable
...
- (void)updateDataSource{
    ...
    if ([self.dataSource respondsToSelector:@selector(dataArrayForTableView:)]) {
        [tempArray addObjectsFromArray:[self.dataSource dataArrayForTableView:self]];
    }
    //判断原始数据源是否为嵌套数组
...
- (BOOL)checkIfSnapshotMeetsEdge{
    ...
    if (minY < self.contentOffset.y) {
        _autoScrollDirection = BBSnapshotMeetsEdgeTop;
        return YES;
    }
    if (maxY > self.bounds.size.height + self.contentOffset.y) {
        _autoScrollDirection = BBSnapshotMeetsEdgeBottom;
        return YES;
    }
    return NO;
}
...
- (void)startAutoScroll{
    ...
    if (_autoScrollDirection == BBSnapshotMeetsEdgeTop) {//向下滚动
...
-(BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch{
    //判断如果是百度地图的view 既可以实现手势拖动;
//    BXLog(@"%@",[touch.view class]);
